...
class Dec_Block1(nn.Module):
    def __init__(self, block_type_ls, in_c, out_c, lat_c, Nx, Ny, k, modes, is_lat, is_d, is_up, num_groups=1, factor=2):
        ...
        if is_up==True:
            self.up_sample = nn.ConvTranspose2d(out_c,out_c,kernel_size=3,stride=2)
        else:
...
class Unet(nn.Module):
    def __init__(self,par):
        ...
        self.enc2 = Enc_Block1(block_type_ls, n_channels, 2*n_channels, 2*n_channels, Nx//2, Ny//2, k, [16,24], is_down=True, is_lat=True)                        # [B,2C,X/2,Y/2]
        ...
        self.dec2 = Dec_Block1(block_type_ls, 2*n_channels, 1*n_channels, 2*n_channels, Nx//2, Ny//2, k, [16,24], is_lat=True, is_d=True, is_up=True)             # [B,C,X,Y]
